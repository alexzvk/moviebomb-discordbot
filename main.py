    ...
    async def on_message(self, message):
        ...
        if self.turns == 0:
            ...
            try:
                ...
                self.timer_message = await message.channel.send(f'Movie {json.loads(response.text)["results"][0]["title"]} is valid. Game will end <t:{remaining_time}:R>')
            except:
        ...
        # odd numbered turns we check if an actor is in a particular movie
            ...
            self.timer_message = await message.channel.send(f'Actor {actor_name} is valid. Game will end <t:{remaining_time}:R>')
        # even numbered turns we check if movie has particular actor
            ...
            self.timer_message = await message.channel.send(f'Movie {movie_name} is valid. Game will end <t:{remaining_time}:R>')
